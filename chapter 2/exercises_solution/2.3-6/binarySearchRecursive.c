...
int binarySearchRecursive(int* A ,int s , int e,int value);
...
int main(){
	...
	int test;
	scanf("%d",&test);
	int index = binarySearchRecursive(A,0,size(A),test);
...
int binarySearchRecursive(int* A ,int s, int e , int value){
	...
	else if(*(A+mid) < value) return binarySearchRecursive(A,mid+1,e,value);
	else return binarySearchRecursive(A,s,mid-1,value);
	return -1;	
}
